...
class QpdInput:
    ...
    # Tax-year selector for the versioned TaxRuleSet. Defaults to 2026 for
    # backward compatibility with existing callers; backend callers pass the
    # persisted request tax_year explicitly.
    tax_year: int = 2026
...
def calculate_qpd(data: QpdInput) -> QpdResult:
    ...
    rules = get_tax_rules(data.tax_year)

    # User-supplied rates remain supported for backward compatibility and
    # deliberate overrides. When the caller uses the registry defaults, the
    # values are identical to the selected tax year's verified rule set.
    tax_rate = _d(data.tax_rate)
    aids_levy_rate = _d(data.aids_levy_rate)

    cumulative_percentages = rules.qpd_cumulative_percentages
    if data.quarter not in cumulative_percentages:
    ...
    schedule = [
        QpdInstalment(
            f"Q{q} - {rules.qpd_rules[q].payment_date}",
            float(rules.qpd_rules[q].installment_percentage),
            _money(total_tax_usd * rules.qpd_rules[q].installment_percentage),
            _money(total_tax_zig * rules.qpd_rules[q].installment_percentage),
        )
        for q in (1, 2, 3, 4)
    ]
    ...
    cumulative_pct = cumulative_percentages[data.quarter]
    ...
    return QpdResult(
        ...
        due_date=rules.qpd_payment_dates[data.quarter],
